...
Автоматизированное обновление A‑записи домена в Яндекс 360 через API создано для доступа к хосту с динамическим белым IP‑адресом (без DynDNS и подобного). 

Скрипт определяет текущий публичный IP, сравнивает его с текущим значением A‑записи и обновляет запись только при расхождении.

> Важно! Если провайдер отдает серый WAN-адрес (например, 192.168.x.x, 10.x.x.x, 172.16.x.x–172.31.x.x)  — это значит, что используется дополнительный NAT. В такой схеме классический проброс портов на вашем марштизаторе не будет работать.  

Описание для работы со скриптом подготовлено для Debian, но адаптируется под любые системы.
