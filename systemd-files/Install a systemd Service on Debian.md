Установка и запуск сервиса DNS‑Automation (используйте файлы из репозитория):

> Исправьте пользователя и пусть в файле ~/dns-automation/systemd-files/dns-automation.service
> Проверьте, что все пути корректны; если нужен запуск от root — удалите строки `User` в `dns-automation.service`.
